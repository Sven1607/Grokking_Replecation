...
def imshow(tensor, renderer=None, xaxis="", yaxis="", **kwargs):
    px.imshow(utils.to_numpy(tensor), color_continuous_midpoint=0.0, color_continuous_scale="RdBu", labels={"x":xaxis, "y":yaxis}, **kwargs).write_image(kwargs["title"] +".png")
def line(tensor, renderer=None, xaxis="", yaxis="", **kwargs):
    title = kwargs["title"] +".png"
    px.line(utils.to_numpy(tensor), labels={"x":xaxis, "y":yaxis}, **kwargs).write_image(title)

def scatter(x, y, xaxis="", yaxis="", caxis="", renderer=None, **kwargs):
    x = utils.to_numpy(x)
    y = utils.to_numpy(y)
    px.scatter(y=y, x=x, labels={"x":xaxis, "y":yaxis, "color":caxis}, **kwargs).write_image(kwargs["title"] +".png")
...
line([train_losses[::100], test_losses[::100]], x=np.arange(0, len(train_losses), 100), xaxis="Epoch", yaxis="Loss", log_y=True, title="TrainingCurveforModularAddition", line_labels=['train', 'test'])
...
imshow(cache["pattern", 0][:, 0, -1, 0].reshape(p, p), title="attention head 0 a to equals", xaxis="b", yaxis="a")

imshow(
    einops.rearrange(cache["pattern", 0][:, :, -1, 0], "(a b) head -> head a b", a=p, b=p),
    title="Attention for Head 0 from a to equals", xaxis="b", yaxis="a", facet_col=0)
...
imshow(fourier_basis, xaxis="Input", yaxis="Component", y=fourier_basis_names, title="Fourier Basis for Modular Addition")
...
imshow(fourier_basis[94][None, :] * fourier_basis[94][:, None], title="Cos 47a times cos 47b")

imshow(fourier_basis[94][None, :] * fourier_basis[0][:, None], title="Cos 47a times const")
...
line((W_logit[neurons_17] @ fourier_basis.T).norm(dim=0), x=fourier_basis_names, title="W logit for freq 17 neurons in the Fourier Basis")
...
line(neurons_sin_17, title = "Logit Weights for sin 17")
...
def plot_embed_bars(
    ...
    **kwargs
    ...
    fig.write_image("embed_bars.png")
